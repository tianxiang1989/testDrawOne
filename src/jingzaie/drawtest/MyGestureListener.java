...
public class MyGestureListener extends GestureDetector.SimpleOnGestureListener {
    ...
    private float targetleft = 430; //目标x轴左距离
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2,
        ...
        if (chartRect.getLeft() > targetleft || chartRect.getRight() < targetright) {
            chartRect.setX(chartRect.getX() + (-1 * distanceX) * (float) sping1);
        } else {
        ...
        private void tandong(int flag1) {
            ...
            if (flag1 == 0) {
                ...
                while (true) {
                    ...
                    if (dx <= 0) {
                        dx = 0;
                        chartRect.setX(0 - (chartRect.getWidth() - (myView.getWidth() - springLength - margin)));
                    } else {
                        chartRect.setX(chartRect.getX() + vx);
                    }
                    chartRect.setLeft(chartRect.getX());
